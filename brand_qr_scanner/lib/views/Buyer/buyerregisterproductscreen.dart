...
import 'package:enQRsure/views/Buyer/buyercounterfeitproductsceen.dart';
...
class BuyerRegisterProductScreen extends StatefulWidget {
  final User user;
  const BuyerRegisterProductScreen({Key? key, required this.user})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => _BuyerRegisterProductScreenState();
}

class _BuyerRegisterProductScreenState
    extends State<BuyerRegisterProductScreen> {
                ...
                children: [
                  ...
                  Column(
                      ...
                      children: [
                        ...
                        Text(
                            df.format(
                                DateTime.parse(product.productDate.toString())),
                            style: GoogleFonts.montserrat(
                                fontSize: 16, color: Colors.black)),
                        const SizedBox(height: 10),
                        Text("Warranty:",
                        ...
                        Text("Buyer:",
                            style: GoogleFonts.montserrat(
                                ...
                                fontWeight: FontWeight.bold,
                                color: Colors.black)),
                        Text(product.buyerName?.toString() ?? 'N/A',
                            style: GoogleFonts.montserrat(
                                fontSize: 16, color: Colors.black)),
                        const SizedBox(height: 10),
                        Container(
  ...
  void registerProduct(Product product) {
    if (widget.user.id == "0") {
      ...
      if (product.buyerId.toString() == '0') {
        String buyerregdate = df.format(DateTime.parse(datetime.toString()));
        ...
        Fluttertoast.showToast(
          msg: "Counterfeit Product Detected",
          toastLength: Toast.LENGTH_SHORT,
          gravity: ToastGravity.BOTTOM,
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (BuildContext context) => BuyerCounterfeitProductScreen(
