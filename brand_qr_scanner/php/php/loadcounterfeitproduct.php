...
$sqlloadcounterfeitproduct = "SELECT cp.*, GROUP_CONCAT(DISTINCT buyer.user_name) AS cproduct_buyername
    FROM tbl_counterfeitproduct AS cp
    LEFT JOIN tbl_user AS buyer ON cp.cproduct_buyerid = buyer.user_id
    WHERE cp.cproduct_name LIKE '%$search%' 
    GROUP BY cp.cproduct_id
    ORDER BY cp.cproduct_submitdate DESC";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $counterfeitproductlist['cproduct_buyername'] = $row['cproduct_buyername'];
